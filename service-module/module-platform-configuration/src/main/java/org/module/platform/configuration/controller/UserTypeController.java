...
import org.module.platform.configuration.service.UserTypeService;
...
public class UserTypeController {
	...
	* @description 人群类型列表
	*/
	@RequiresAuthentication(level=Level.OPERATION, value = { "platform:configure:userType:list" })
	...
	* @description 新增人群类型
	*/
	@RequiresAuthentication(level=Level.OPERATION, value = { "platform:configure:userType:insert" })
	...
	* @description 修改人群类型
	*/
	@RequiresAuthentication(level=Level.OPERATION, value = { "platform:configure:userType:update" })
	@RequestMapping(value = { "/user/type/{id}" }, method = RequestMethod.PUT)
	public JsonApi userTypeUpdate(@PathVariable("id")Integer id,
			@Validated({ BaseEntity.Update.class }) @RequestBody UserType userType,
	...
	* @date 2018年3月23日
	* @version 1.0
	* @description 启用禁用人群类型
	*/
	@RequiresAuthentication(level=Level.OPERATION, value = { "platform:configure:userType:isUsed" })
	@RequestMapping(value = { "/user/type/is/used/{id}" }, method = RequestMethod.PUT)
	public JsonApi userTypeIsUsed(@PathVariable("id")Integer id, @Validated({ BaseEntity.Update.class }) @RequestBody UserType userType, BindingResult result) {
		userType.setId(id);
		//数据是否存在
		Map<String, Object> userTypeOneMap = userTypeService.getOne(userType);
		if (MapUtils.isEmpty(userTypeOneMap)) {
	...
	* @description 人群类型详情
	*/
	@RequiresAuthentication(level=Level.OPERATION, value = { "platform:configure:userType:detail" })
	...
	* @description 删除人群类型
	*/
	@RequiresAuthentication(level=Level.OPERATION, value = { "platform:configure:userType:delete" })
	@RequestMapping(value = { "/user/type/{id}" }, method = RequestMethod.DELETE)
	public JsonApi userTypeDelete(@PathVariable("id")Integer id,@Validated({ BaseEntity.Delete.class })  UserType userType,BindingResult result) {
		userType.setId(id);
