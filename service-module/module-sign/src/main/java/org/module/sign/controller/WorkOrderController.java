...
import org.module.sign.global.BaseGlobal;
import org.module.sign.global.Sign;
import org.module.sign.global.Sign.WorkOrderStatus;
import org.module.sign.message.Prompt;
import org.module.sign.service.UserServiceService;
import org.module.sign.service.WorkOrderLogService;
import org.module.sign.service.WorkOrderService;
...
public class WorkOrderController {
	...
	*/
	@Transactional
	@RequiresAuthentication(authc = true, value = { "sign:workOrder:insert" })
	...
	public JsonApi insert(@RequestBody @Validated({ BaseEntity.Insert.class })  WorkOrder workOrder,BindingResult result,
		...
		Integer[] statuss = new Integer[]{Sign.WorkOrderStatus.PENDINGRESPONSE.getValue(),
				Sign.WorkOrderStatus.TOBEEXECUTED.getValue(),
				Sign.WorkOrderStatus.INTHEEXECUTION.getValue()
			};
		...
		workOrder.setOrderSources(Sign.WorkOrderStatus.ORDERRESOURCEONLINEWECHAT.getValue());
		workOrder.setStatus(Sign.WorkOrderStatus.PENDINGRESPONSE.getValue());
		...
		if(workOrderService.insert(workOrder) > 0){
			...
			if(userServiceService.updateLockTime(userService) > 0){
				...
				workOrderLog.setLogType(Sign.WorkOrderStatus.WORKORDERLOGTYPEOPERATIONLOG.getValue());
				workOrderLog.setLaunchType(Sign.WorkOrderStatus.WORKORDERLOGTYPEUSERSPONSOR.getValue());
				workOrderLog.setWorkOrderId(workOrder.getId());
				workOrderLog.setStatus(Sign.WorkOrderStatus.PENDINGRESPONSE.getValue());
	...
	*/
	...
	public JsonApi getList(@Validated({ BaseEntity.SelectAll.class })  WorkOrder workOrder,BindingResult result){
		...
		// 查询用户工单列表
		List<Map<String, Object>> workOrdersResultList = workOrderService.getListByUserId(workOrder);
	...
	*/
	...
	public JsonApi updateUserWorkOrder(@PathVariable("id")Integer id,
		...
		workOrder.setUpdateDate(new Date());
		workOrder.setStatus(Sign.WorkOrderStatus.EVALUATED.getValue());
		//用户评价工单
		if (workOrderService.updateUserWorkOrder(workOrder) > 0) {
			...
			workOrderLog.setStatus(Sign.WorkOrderStatus.EVALUATED.getValue());
			workOrderLog.setCreateDate(new Date());
			if(workOrderLogService.insert(workOrderLog) > 0){
				return new JsonApi(ApiCode.OK);
			}
