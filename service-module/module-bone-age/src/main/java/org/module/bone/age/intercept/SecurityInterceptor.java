package org.module.bone.age.intercept;
...
import org.module.bone.age.intercept.permission.IOrganizationUserRoleService;
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)
		...
		if (handler instanceof HandlerMethod) {
			...
			if (authentication.ignore()) {
				...
				case ROLE:
					/* 获取角色列表 */
					JsonApi organizationUserRoleRolesList = organizationUserRoleService.getOrganizationUserRoleRolesList(1,1000, token, Integer.valueOf(organizationTeamId) );
					if (organizationUserRoleRolesList.getCode() != ApiCode.OK.getValue()) {
						/* 发生错误返回错误消息 */
						response.getWriter().write(JSON.toJSONString(new JsonApi(ApiCode.FORBIDDEN).setMsg("organization.user.forbidden")));
						return false;
					}
				...
				case PERMISSION:
					/* 获取权限列表 */
					JsonApi organizationUserRolePermissionList = organizationUserRoleService.getOrganizationUserRolepermissionsList(1,1000, token, Integer.valueOf(organizationTeamId) );
				...
				case OPERATION:
					/* 获取操作列表 */
					JsonApi organizationUserRoleOperationList = organizationUserRoleService.getOrganizationUserRoleOperationsList(1,1000, token, Integer.valueOf(organizationTeamId) );
